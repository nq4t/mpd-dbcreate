...
void sacd_disc_t::get_info(uint32_t track_index, TagHandler& handler) {
	...
	if (!sb->master_text.album_title.empty()) {
		// Just use clean album title
		handler.OnTag(TAG_ALBUM, sb->master_text.album_title.c_str());
	}
	...
	if (!area->area_track_text[track_index].track_type_title.empty()) {
		// Just use the clean title without prefixes
		handler.OnTag(TAG_TITLE, area->area_track_text[track_index].track_type_title.c_str());
		
		// Store channel info in a comment tag instead for filtering purposes
		tag_value = (get_track_area_id() == AREA_TWOCH) ? "SACD_2CH" : "SACD_MCH";
		handler.OnTag(TAG_COMMENT, tag_value.c_str());
	}
