				...
				if (p_curr[3] == 0xbd) { // check for private stream 1
					if (p_curr < p_block + DVD_BLOCK_SIZE - 9) {
						...
						if (p_ps1_header < p_ps1_end && p_ps1_end <= p_block + DVD_BLOCK_SIZE) {
							int ps1_header_length = get_ps1_info_length(p_ps1_header, (int)(p_ps1_end - p_ps1_header));
							if (p_ps1_info && p_ps1_info->header.stream_id == UNK_STREAM_ID && ps1_header_length > 0)
								std::memcpy(p_ps1_info, p_ps1_header, ps1_header_length < (int)sizeof(sub_header_t) ? ps1_header_length : (int)sizeof(sub_header_t));
							p_ps1_body = p_ps1_header + ps1_header_length;
							int ps1_body_length = (int)(p_ps1_end - p_ps1_body);
							if (ps1_body_length > 0) {
								std::memcpy(p_ps1_buffer + *p_ps1_offset + ps1_offset, p_ps1_body, ps1_body_length);
